...
function RequestsManager() {
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    ...
    try {
      ...
      } else alert("تعذر تحديث الحالة من السيرفر");
...
function AdminDashboard({ onLogout }) {
  ...
  const downloadReport = async () => {
    ...
    } catch (err) { alert("حدث خطأ أثناء تحميل التقرير"); }
  };
            ...
            {pendingReceipts.map(rec => (
              <div key={rec.id} style={{background:C.surf2, borderRadius:12, padding:16, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16}}>
                <div style={{display:"flex", alignItems:"center", gap:16}}>
                  <div onClick={() => setSelectedImage(rec.image)} style={{width:50, height:50, background:C.border, borderRadius:8, cursor:"zoom-in", backgroundImage:`url(${rec.image})`, backgroundSize:'cover', backgroundPosition:'center', border:`1px solid ${C.muted}`}} title="اضغط لتكبير الإيصال" />
                  <div>
